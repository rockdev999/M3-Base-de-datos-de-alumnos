...
let alumnosP_fisi = JSON.parse(localStorage.getItem("alumnosP_fisi")) || [];
...
agregarAlumnosButton.addEventListener("click", () => {
  alumnos.forEach((alumno, index) => {
    ...
    if (checkbox.checked) {
      alumnosP_fisi.push(alumno);
    }
  });
  console.log(alumnosP_fisi);
});

const showStudentsPrincipal = (alumnosP_fisi) => {
  let llistAlumnosHTML = "";
  let i = 0;
  alumnosP_fisi.forEach((alumno) => {
    llistAlumnosHTML += `
      <tr>
                  <th scope="row">${i + 1}</th>
                  <td>${alumno.ci}</td>
  ...
  localStorage.setItem("alumnosP_fisi", JSON.stringify(alumnosP_fisi));
};

listaAluButton.addEventListener("click", function () {
  showStudentsPrincipal(alumnosP_fisi);
  promedioCursoT.innerHTML = `${promedioCurso(alumnosP_fisi).toFixed(1)}`;
});

function actualizaNotasAlu(alumnosP_fisi) {
  let i = 0;
  alumnosP_fisi.forEach((alumno, i) => {
...
guardarnotas.addEventListener("click", (event) => {
  actualizaNotasAlu(alumnosP_fisi);
  localStorage.setItem("alumnosP_fisi", JSON.stringify(alumnosP_fisi));
  console.log("adasdsa");
  console.log(alumnosP_fisi);
  showStudentsPrincipal(alumnosP_fisi);
...
ordAsc.addEventListener("click", (event) => {
  alumnosP_fisi = ordenarNotasAsc(alumnosP_fisi);
  showStudentsPrincipal(alumnosP_fisi);
});

ordDsc.addEventListener("click", (event) => {
  alumnosP_fisi = ordenarNotasDsc(alumnosP_fisi);
  showStudentsPrincipal(alumnosP_fisi);
});
