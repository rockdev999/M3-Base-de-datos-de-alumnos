...
let alumnosP_mate = JSON.parse(localStorage.getItem("alumnosP_mate")) || [];
...
agregarAlumnosButton.addEventListener("click", () => {
  alumnos.forEach((alumno, index) => {
    ...
    if (checkbox.checked) {
      alumnosP_mate.push(alumno);
    }
  });
  console.log(alumnosP_mate);
});

const showStudentsPrincipal = (alumnosP_mate) => {
  let llistAlumnosHTML = "";
  let i = 0;
  alumnosP_mate.forEach((alumno) => {
    llistAlumnosHTML += `
      <tr>
                  <th scope="row">${i + 1}</th>
                  <td>${alumno.ci}</td>
  ...
  localStorage.setItem("alumnosP_mate", JSON.stringify(alumnosP_mate));
};

listaAluButton.addEventListener("click", function () {
  showStudentsPrincipal(alumnosP_mate);
  promedioCursoT.innerHTML = `${promedioCurso(alumnosP_mate).toFixed(1)}`;
});

function actualizaNotasAlu(alumnosP_mate) {
  let i = 0;
  alumnosP_mate.forEach((alumno, i) => {
...
guardarnotas.addEventListener("click", (event) => {
  actualizaNotasAlu(alumnosP_mate);
  localStorage.setItem("alumnosP_mate", JSON.stringify(alumnosP_mate));
  console.log("adasdsa");
  console.log(alumnosP_mate);
  showStudentsPrincipal(alumnosP_mate);
...
ordAsc.addEventListener("click", (event) => {
  alumnosP_mate = ordenarNotasAsc(alumnosP_mate);
  showStudentsPrincipal(alumnosP_mate);
});

ordDsc.addEventListener("click", (event) => {
  alumnosP_mate = ordenarNotasDsc(alumnosP_mate);
  showStudentsPrincipal(alumnosP_mate);
});
