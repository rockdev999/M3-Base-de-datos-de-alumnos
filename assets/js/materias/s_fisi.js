...
let alumnosS_fisi = JSON.parse(localStorage.getItem("alumnosS_fisi")) || [];
...
agregarAlumnosButton.addEventListener("click", () => {
  alumnos.forEach((alumno, index) => {
    ...
    if (checkbox.checked) {
      alumnosS_fisi.push(alumno);
    }
  });
  console.log(alumnosS_fisi);
});

const showStudentsPrincipal = (alumnosS_fisi) => {
  let llistAlumnosHTML = "";
  let i = 0;
  alumnosS_fisi.forEach((alumno) => {
    llistAlumnosHTML += `
      <tr>
                  <th scope="row">${i + 1}</th>
                  <td>${alumno.ci}</td>
  ...
  localStorage.setItem("alumnosS_fisi", JSON.stringify(alumnosS_fisi));
};

listaAluButton.addEventListener("click", function () {
  showStudentsPrincipal(alumnosS_fisi);
  promedioCursoT.innerHTML = `${promedioCurso(alumnosS_fisi).toFixed(1)}`;
});

function actualizaNotasAlu(alumnosS_fisi) {
  let i = 0;
  alumnosS_fisi.forEach((alumno, i) => {
...
guardarnotas.addEventListener("click", (event) => {
  actualizaNotasAlu(alumnosS_fisi);
  localStorage.setItem("alumnosS_fisi", JSON.stringify(alumnosS_fisi));
  console.log("adasdsa");
  console.log(alumnosS_fisi);
  showStudentsPrincipal(alumnosS_fisi);
...
ordAsc.addEventListener("click", (event) => {
  alumnosS_fisi = ordenarNotasAsc(alumnosS_fisi);
  showStudentsPrincipal(alumnosS_fisi);
});

ordDsc.addEventListener("click", (event) => {
  alumnosS_fisi = ordenarNotasDsc(alumnosS_fisi);
  showStudentsPrincipal(alumnosS_fisi);
});
