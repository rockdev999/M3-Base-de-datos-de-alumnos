...
let alumnosP_quimi = JSON.parse(localStorage.getItem("alumnosP_quimi")) || [];
...
agregarAlumnosButton.addEventListener("click", () => {
  alumnos.forEach((alumno, index) => {
    ...
    if (checkbox.checked) {
      alumnosP_quimi.push(alumno);
    }
  });
  console.log(alumnosP_quimi);
});

const showStudentsPrincipal = (alumnosP_quimi) => {
  let llistAlumnosHTML = "";
  let i = 0;
  alumnosP_quimi.forEach((alumno) => {
    llistAlumnosHTML += `
      <tr>
                  <th scope="row">${i + 1}</th>
                  <td>${alumno.ci}</td>
  ...
  localStorage.setItem("alumnosP_quimi", JSON.stringify(alumnosP_quimi));
};

listaAluButton.addEventListener("click", function () {
  showStudentsPrincipal(alumnosP_quimi);
  promedioCursoT.innerHTML = `${promedioCurso(alumnosP_quimi).toFixed(1)}`;
});

function actualizaNotasAlu(alumnosP_quimi) {
  let i = 0;
  alumnosP_quimi.forEach((alumno, i) => {
...
guardarnotas.addEventListener("click", (event) => {
  actualizaNotasAlu(alumnosP_quimi);
  localStorage.setItem("alumnosP_quimi", JSON.stringify(alumnosP_quimi));
  console.log("adasdsa");
  console.log(alumnosP_quimi);
  showStudentsPrincipal(alumnosP_quimi);
...
ordAsc.addEventListener("click", (event) => {
  alumnosP_quimi = ordenarNotasAsc(alumnosP_quimi);
  showStudentsPrincipal(alumnosP_quimi);
});

ordDsc.addEventListener("click", (event) => {
  alumnosP_quimi = ordenarNotasDsc(alumnosP_quimi);
  showStudentsPrincipal(alumnosP_quimi);
});
