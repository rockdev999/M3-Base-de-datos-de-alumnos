...
let alumnosS_quimi = JSON.parse(localStorage.getItem("alumnosS_quimi")) || [];
...
agregarAlumnosButton.addEventListener("click", () => {
  alumnos.forEach((alumno, index) => {
    ...
    if (checkbox.checked) {
      alumnosS_quimi.push(alumno);
    }
  });
  console.log(alumnosS_quimi);
});

const showStudentsPrincipal = (alumnosS_quimi) => {
  let llistAlumnosHTML = "";
  let i = 0;
  alumnosS_quimi.forEach((alumno) => {
    llistAlumnosHTML += `
      <tr>
                  <th scope="row">${i + 1}</th>
                  <td>${alumno.ci}</td>
  ...
  localStorage.setItem("alumnosS_quimi", JSON.stringify(alumnosS_quimi));
};

listaAluButton.addEventListener("click", function () {
  showStudentsPrincipal(alumnosS_quimi);
  promedioCursoT.innerHTML = `${promedioCurso(alumnosS_quimi).toFixed(1)}`;
});

function actualizaNotasAlu(alumnosS_quimi) {
  let i = 0;
  alumnosS_quimi.forEach((alumno, i) => {
...
guardarnotas.addEventListener("click", (event) => {
  actualizaNotasAlu(alumnosS_quimi);
  localStorage.setItem("alumnosS_quimi", JSON.stringify(alumnosS_quimi));
  console.log("adasdsa");
  console.log(alumnosS_quimi);
  showStudentsPrincipal(alumnosS_quimi);
...
ordAsc.addEventListener("click", (event) => {
  alumnosS_quimi = ordenarNotasAsc(alumnosS_quimi);
  showStudentsPrincipal(alumnosS_quimi);
});

ordDsc.addEventListener("click", (event) => {
  alumnosS_quimi = ordenarNotasDsc(alumnosS_quimi);
  showStudentsPrincipal(alumnosS_quimi);
});
